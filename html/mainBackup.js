...
function create(){
  // Score.
  scoreText = this.add.text(config.width/2-50, 5, "SCORE: " + currentScore);
  // Check if mobile or desktop.
  if (!this.sys.game.device.input.touch){
    this.cursors = this.input.keyboard.createCursorKeys();
    this.key_r = this.input.keyboard.addKey(Phaser.Keyboard.D);
  } else {
  ...
  let Snake = new Phaser.Class({
    ...
    move: function(time){
      ...
      if (hitBody){
        this.alive = false;
        scoreText.text = "YOU DIED. SCORE: " + currentScore;
        return false;
      } else {
